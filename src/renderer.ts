...
const statusJsonEl = getRequiredElement<HTMLDivElement>("status-json");
...
function setStatus(
  ...
  const statusEl = target === "json" ? statusJsonEl : statusCronEl;
...
function formatJson(): void {
  try {
    ...
    setStatus("json", `格式化失败：${getErrorMessage(error)}`, true);
  }
}
...
function minifyJson(): void {
  try {
    ...
    setStatus("json", `压缩失败：${getErrorMessage(error)}`, true);
  }
}
...
async function copyOutput(): Promise<void> {
  ...
  if (!content) {
    setStatus("json", "没有可复制的内容。", true);
    return;
  }

  try {
    await navigator.clipboard.writeText(content);
    setStatus("json", "输出内容已复制。");
  } catch (error: unknown) {
    setStatus("json", `复制失败：${getErrorMessage(error)}`, true);
  }
}
...
function fillSample(): void {
  ...
  setStatus("json", "示例 JSON 已填充。");
}

function switchTab(target: "json" | "cron"): void {
  const jsonActive = target === "json";
  tabJsonBtn.classList.toggle("active", jsonActive);
  tabCronBtn.classList.toggle("active", !jsonActive);
  panelJsonEl.classList.toggle("active", jsonActive);
  panelCronEl.classList.toggle("active", !jsonActive);
}
...
function validateCronExpression(): void {
  try {
    ...
    setStatus("cron", "Cron 表达式校验通过。");
  } catch (error: unknown) {
    setStatus("cron", `Cron 校验失败：${getErrorMessage(error)}`, true);
  }
}
...
function previewCronRuns(): void {
  try {
    ...
    cronOutputEl.value = lines.join("\n");
    setStatus("cron", "已生成未来执行时间。");
  } catch (error: unknown) {
    setStatus("cron", `预览失败：${getErrorMessage(error)}`, true);
  }
}
...
function buildCronFromFields(): void {
  try {
    ...
    renderCronValidation(parsed);
    setStatus("cron", "已从字段生成并校验 Cron 表达式。");
  } catch (error: unknown) {
    setStatus("cron", `生成失败：${getErrorMessage(error)}`, true);
  }
}
